...
// public class JdbcTest3 extends TestCase {
...
"Ten Things to Know About ModelOps.pdf", 
	...
	@Test
	public void testSimple4() throws SQLException, JsonMappingException, JsonProcessingException {
		...
		Collection<Chunk> list = this.runChunkStatement("SELECT * FROM vector_store WHERE metadata->>'source' = 'FreeBSD 12.2 Handbook.pdf';");
		chunk_count = list.size();
		...
		while(listiter.hasNext()) {
			...
			// System.out.println(metadata.get("total_chunks"));
			// System.out.println(metadata.get("chunk_index"));
			total_chunks = Integer.parseInt(metadata.get("total_chunks").toString());
			chunk_index = Integer.parseInt(metadata.get("chunk_index").toString());
			if( total_chunks != chunk_count ) {
				System.out.println(" !!! DIFFERING LENGTHS !! ");
				break;
			}
			if( chunk_indexes.contains(chunk_index) ) {
				System.out.println(" !!! CHUNK PRESENT !! ");
				break;
			}
		...
		chunk_count = chunk_indexes.size();
		System.out.println(" OK ");

	}

	public boolean checkDocument(String document) throws SQLException, JsonMappingException, JsonProcessingException {
		...
		if( list == null ) {
			System.out.println(" Chunk check failed for document: " + document + ", no chunks found");
			return false;
		}
		...
		while( listiter.hasNext() ) {
			...
			if( total_chunks != chunk_count ) {
				System.out.println(" Chunk check failed for document: " + document + ", chunk count mismatch: found: " + chunk_count + ", total: " + total_chunks + ", index: " + chunk_index);
				return false;
			}

			if( chunk_indexes.contains(chunk_index) ) {
				System.out.println(" Chunk check failed for document: " + document + ", chunk count mismatch: index already present: " + chunk_index);
				return false;
			}
		...
		if( chunk_indexes.size() != chunk_count ) {
			System.out.println(" Chunk check failed for document: " + document + ", chunk count mismatch: found: " + chunk_count + ", indexes: " + chunk_indexes.size());
			return false;
		}

		if( chunk_indexes.size() != total_chunks ) {
			System.out.println(" Chunk check failed for document: " + document + ", chunk count mismatch: total: " + total_chunks + ", indexes: " + chunk_indexes.size());
			return false;
		}
	...
	@Test
	public void testSimple5() throws SQLException, JsonMappingException, JsonProcessingException {

		String[] documents = this.getDocumentNames();

		for( String document : documents ) {
			boolean check = this.checkDocument(document);
			if( !check ) {
				System.out.println(" Chunk check failed for document: " + document );
			}
		}
	...
	@Test
	public void testSimple6() throws SQLException, JsonMappingException, JsonProcessingException {
